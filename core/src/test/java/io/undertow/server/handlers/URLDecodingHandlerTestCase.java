...
public class URLDecodingHandlerTestCase {
    ...
    @Test
    public void testDecodeSlashesInMatchedPaths() throws Exception {
        // When UndertowOptions.DECODE_URL is disabled, the URLDecodingHandler should decode values.
        Undertow undertow = Undertow.builder()
                .setServerOption(UndertowOptions.DECODE_URL, false)
                .addHttpListener(PORT, "0.0.0.0")
        ...
        try {
            ...
            try (CloseableHttpResponse response = client.execute(
                    new HttpGet("http://localhost:" + PORT + "/api/test%2Ftest/tail"))) {
                Assert.assertEquals("test/test", getResponseString(response));
            }
        } finally {
            undertow.stop();
        }
