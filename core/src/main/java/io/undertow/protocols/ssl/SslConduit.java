...
public class SslConduit implements StreamSourceConduit, StreamSinkConduit {
     ...
     */
    private int engineUnwrap(ByteBuffer src, ByteBuffer[] dsts, int off, int len) throws IOException {
        boolean bytesProduced = false;
        SSLEngineResult result;
        do {
            result = engine.unwrap(src, dsts, off, len);
            bytesProduced = bytesProduced || result.bytesProduced() > 0;
        ...
        return packSSLEngineResult(result.getStatus(), result.getHandshakeStatus(), bytesProduced);
    }

    /**
     * Resulting value is packed thusly:
     * <ul>
     *     <li>Bit 00..7: SSLEngineResult.Status ordinal</li>
     *     <li>Bit 8..15: SLEngineResult.HandshakeStatus ordinal</li>
     *     <li>Bit 16: 1 = SSLEngineResult.bytesProduced() > 0; 0 = no bytes produced</li>
     * </ul>
     */
    static int packSSLEngineResult(SSLEngineResult.Status status, SSLEngineResult.HandshakeStatus handshakeStatus, boolean bytesProduced) {
        int result = status.ordinal();
        result |= (handshakeStatus.ordinal() << 8);
        result |= ((bytesProduced ? 1 : 0) << 16);
        return result;
    }

    private static final SSLEngineResult.Status[] ENGINE_RESULT_STATUSES = SSLEngineResult.Status.values();
    private static final SSLEngineResult.HandshakeStatus[] ENGINE_RESULT_HANDSHAKE_STATUSES = SSLEngineResult.HandshakeStatus.values();

    private static SSLEngineResult.Status unpackSSLEngineResultStatus(int packed) {
        int ordinal = packed & 0xFF;
        return ENGINE_RESULT_STATUSES[ordinal];
    }

    private static SSLEngineResult.HandshakeStatus unpackSSLEngineResultHandshakeStatus(int packed) {
        int ordinal = (packed >>> 8) & 0xFF;
        return ENGINE_RESULT_HANDSHAKE_STATUSES[ordinal];
    }

    private static boolean unpackSSLEngineResultBytesProduced(int packed) {
    ...
    private synchronized long doUnwrap(ByteBuffer[] userBuffers, int off, int len) throws IOException {
        ...
        try {
            ...
            try {
                if (userBuffers != null) {
                    result = engineUnwrap(this.dataToUnwrap.getBuffer(), userBuffers, off, len);
                    if (unpackSSLEngineResultStatus(result) == SSLEngineResult.Status.BUFFER_OVERFLOW) {
                        ...
                        result = engineUnwrap(this.dataToUnwrap.getBuffer(), d, 0, d.length);
                    ...
                    result = engineUnwrap(this.dataToUnwrap.getBuffer(), new ByteBuffer[] {unwrappedData.getBuffer()}, 0, 1);
                }
            } finally {
                if (unwrapBufferUsed) {
            ...
            if (unpackSSLEngineResultStatus(result) == SSLEngineResult.Status.CLOSED) {
                if(dataToUnwrap != null) {
                    dataToUnwrap.close();
                    dataToUnwrap = null;
                }
            ...
            if (!handleHandshakeResult(unpackSSLEngineResultHandshakeStatus(result))) {
                if (this.dataToUnwrap.getBuffer().hasRemaining()
                        && unpackSSLEngineResultStatus(result) != SSLEngineResult.Status.BUFFER_UNDERFLOW
            ...
            if (unpackSSLEngineResultStatus(result) == SSLEngineResult.Status.BUFFER_UNDERFLOW) {
                state &= ~FLAG_DATA_TO_UNWRAP;
            } else if (unpackSSLEngineResultStatus(result) == SSLEngineResult.Status.BUFFER_OVERFLOW) {
            ...
            if (unpackSSLEngineResultBytesProduced(result) || (unwrappedData != null && unwrappedData.isOpen() && unwrappedData.getBuffer().hasRemaining())) {
    ...
    private synchronized long doWrap(ByteBuffer[] userBuffers, int off, int len) throws IOException {
        ...
        try {
            ...
            if (!handleHandshakeResult(result.getHandshakeStatus())) {
                return 0;
            }
    ...
    private boolean handleHandshakeResult(SSLEngineResult.HandshakeStatus handshakeStatus) throws IOException {
